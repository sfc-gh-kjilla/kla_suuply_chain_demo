...
import { ChatPanel, TelemetryChart, KLALogo, MaintenanceHistory, ShippingComparison, PartTransfer, PartsInventory, CustomerFleetOverview, SensorHealthPanel, EscalationCasesPanel, CaseDetailPanel } from './components';
...
function App() {
  ...
  const [activeBottomTab, setActiveBottomTab] = useState<'caseDetail' | 'telemetry' | 'maintenance' | 'shipping' | 'transfer' | 'inventory'>('caseDetail');
  ...
  const openCases = mockEscalationCases.filter(c => c.STATUS !== 'RESOLVED');
  const sev1Count = openCases.filter(c => c.SEVERITY === 'SEV1').length;
  const sev2Count = openCases.filter(c => c.SEVERITY === 'SEV2').length;
  const slaBreachedCount = openCases.filter(c => c.SLA_REMAINING_HOURS < 0).length;
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <header style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <KLALogo size={32} />
            <div>
              <h1 style={{ margin: 0, fontSize: '14px', fontWeight: 600 }}>Escalation Command Center</h1>
              <p style={{ margin: 0, fontSize: '10px', color: colors.textMuted }}>Field Service & Supply Chain Operations</p>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
            ...
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: '10px', color: colors.textMuted }}>OPEN CASES</div>
              <div style={{ fontSize: '20px', fontWeight: 700, color: colors.accent }}>{openCases.length}</div>
            </div>
            <div
              onClick={() => triggerChatPrompt('Show all SEV1 escalation cases with SLA status, assigned engineers, and recommended actions')}
              style={{
                textAlign: 'center',
                cursor: 'pointer',
                padding: '6px 10px',
            ...
            >
              <div style={{ fontSize: '10px', color: colors.textMuted }}>SEV1</div>
              <div style={{ fontSize: '20px', fontWeight: 700, color: colors.critical }}>{sev1Count}</div>
            </div>
            <div
              onClick={() => triggerChatPrompt('Show all SEV2 cases and which are closest to SLA breach')}
              style={{
                textAlign: 'center',
                cursor: 'pointer',
                padding: '6px 10px',
              ...
              onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}
            >
              <div style={{ fontSize: '10px', color: colors.textMuted }}>SEV2</div>
              <div style={{ fontSize: '20px', fontWeight: 700, color: colors.warning }}>{sev2Count}</div>
            </div>
            {slaBreachedCount > 0 && (
              <div
                onClick={() => triggerChatPrompt('List all cases with breached SLAs and recommended escalation actions')}
                style={{
                  textAlign: 'center',
                  cursor: 'pointer',
                  padding: '6px 10px',
                ...
                onMouseEnter={(e) => e.currentTarget.style.background = colors.critical + '20'}
                onMouseLeave={(e) => e.currentTarget.style.background = colors.critical + '10'}
              >
                <div style={{ fontSize: '10px', color: colors.critical }}>SLA BREACH</div>
                <div style={{ fontSize: '20px', fontWeight: 700, color: colors.critical }}>{slaBreachedCount}</div>
              </div>
            )}
            <button
              onClick={toggleTheme}
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', padding: '8px 12px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '8px', flex: '0 0 30%', minHeight: 0, overflow: 'hidden' }}>
              <div style={{
                background: colors.bgSecondary,
                borderRadius: '10px',
                border: `1px solid ${colors.border}`,
                ...
                <h3 style={{ margin: '0 0 6px 0', fontSize: '12px', color: colors.textMuted, flexShrink: 0 }}>
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin: '0 0 6px 0', flexShrink: 0 }}>
                  <h3 style={{ margin: 0, fontSize: '12px', color: colors.textMuted }}>
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px', flexShrink: 0 }}>
                  <div style={{ display: 'flex', gap: '4px' }}>
                    {[
                      { id: 'caseDetail', label: 'Case Detail', icon: <AssignmentIcon style={{ fontSize: 14 }} /> },
                      ...
                      { id: 'shipping', label: 'Source Parts', icon: <CompareArrowsIcon style={{ fontSize: 14 }} /> },
                ...
                <div style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
                  {activeBottomTab === 'caseDetail' && (
                    selectedCase ? (
                      ...
                      <div style={{
                        ...
                        Select an escalation case to view details
                      </div>
                    )
                  )}
                  ...
                  {activeBottomTab === 'shipping' && (
                    <ShippingComparison
                      ...
                      onShip={(option) => {
                        triggerChatPrompt(`Ship part 994-023 from ${option.WAREHOUSE_LOCATION} to ${selectedScanner}. Show landed cost breakdown including tariffs and duties.`);
                      }}
                    />
                  )}
